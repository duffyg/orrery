...
function drawOrbitals (plutoFlag) {
    ...
    var p, np;
    var orbear = PlanetPosition(EAR, dispDate);
    ...
    for (p = 1; p <= np; p++) { // 0 is for Sun, +1 is for Pluto
        ...
        var orbPos = getOrbPos(p, orbPath[p][0], orbear)
...
function getOrbPos (p, orbpos, orbear) {
    ...
    var pos = [orbpos[0], orbpos[1], orbpos[2]]
    if (p !== EAR) {
...
function drawPlanets (plutoFlag) {
    ...
    var orbpos, pos = [], i;
    var orbear, orbsun
    orbear = PlanetPosition(EAR, dispDate);
    orbsun = PlanetPosition(SUN, dispDate);
